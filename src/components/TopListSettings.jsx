...
function TopListSettings({
  ...
  return (
    <section className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 text-lg leading-snug rounded-none gap-4 sm:gap-0">
      ...
      <div className="flex flex-col sm:flex-row gap-4 sm:gap-8 w-full sm:w-auto items-center"> {/* Added items-center */}
        ...
        <div className="flex items-center mt-3 md:mt-0 w-full sm:w-auto"> {/* Adjusted for layout consistency */}
          <div className="relative w-full sm:w-auto"> {/* Ensure select takes available width or auto */}
            <select
              className="appearance-none border border-gray-400 text-[18px] rounded px-5 py-2 pr-8 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 w-full sm:w-auto" // Added w-full sm:w-auto
              value={sortOption}
          ...
          {/* View Mode Buttons - using basic buttons for now, can be replaced by Button component */}
           <button
            ...
            className={`rounded-md h-9 ml-1 px-2.5 ${viewMode === 'list' ?'bg-[rgba(39,73,137,1)] text-white' : 'hover:bg-gray-300   bg-transparent'} border border-gray-300 `}
