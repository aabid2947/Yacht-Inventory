...
function TopListSettings({
  ...
  return (
    <section className="flex flex-col md:flex-row justify-between items-start md:items-center mb-[50px] text-lg leading-snug rounded-none gap-4 sm:gap-0">
      <p className="my-auto text-[#000000A6] text-[18px]">
        {resultsText}
      </p>
      <div className="flex flex-col sm:flex-row gap-4 sm:gap-8 w-full sm:w-auto items-center"> {/* Added items-center */}
        <label className="my-auto text-[#000000A6] text-[18px] whitespace-nowrap"> {/* Added whitespace-nowrap */}
          Sort by:
        </label>
        <div className="flex items-center mt-3 md:mt-0 w-full sm:w-auto"> {/* Adjusted for layout consistency */}
          <div className="relative w-full sm:w-auto"> {/* Ensure select takes available width or auto */}
            <select
              className="appearance-none border border-gray-400 text-[18px] tracking-[0.4px] rounded-[6px] px-4 py-2 pr-8 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 w-full sm:w-auto" // Added w-full sm:w-auto
              value={sortOption}
          ...
          {/* View Mode Buttons - using basic buttons for now, can be replaced by Button component */}
           <button
            ...
            className={`rounded-[6px] h-[40px] w-[40px] ml-[12px] p-[8px] ${viewMode === 'list' ?'bg-[rgba(39,73,137,1)] text-white' : 'hover:bg-gray-300   bg-transparent'} border border-gray-300 `}
          ...
          <button
            ...
            className={`ml-[7px] h-[40px] w-[40px] rounded-[6px] flex items-center justify-center 
